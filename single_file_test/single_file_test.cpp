...
//
// TransferPolicy, for handling switched between task affinities
//
...
struct Continuation
{
    ...
    auto resume(Scheduler* scheduler) noexcept -> std::coroutine_handle<>
    {
...
struct Promise
{
    ...
    auto final_suspend() noexcept
    {
        struct FinalTransitionAwaiter
        {
            ...
            {
                if (promise_->continuation_.index() != 0)
                {
                    // This coroutine was awaited.
                    return std::visit([&](auto&& cont) -> std::coroutine_handle<> {
    ...
    auto await_transform(TaskBase<NextAffinity, NextT>&& next_task) noexcept
    {
        struct TransferAwaiter
        {
            ...
            auto await_suspend(std::coroutine_handle<> awaiting_coroutine) noexcept -> std::coroutine_handle<>
            {
                ...
                promise.continuation_ = Continuation<NextAffinity, Affinity>{ awaiting_coroutine };
